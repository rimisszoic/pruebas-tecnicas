...
import { DETAILS_OF_LISTS } from '../constansts/details-of-lists.js'
...
export const useUserLists = create(persist((set, get) => ({
    ...
    addBook: ({ newBook, listIdTarget, listIdOrigin }) => {
        ...
        const listOrigin = lists.find(list => list.id === listIdOrigin) || lists.find(list => list.books.some(book => book.ISBN === newBook.ISBN))
        const bookIsInList = listToAdd?book?.some(book => book.ISBN === newBook.ISBN)
        ...
        const listsUpdated = lists.map(list => {
            if(listOrigin?.id === list.id) {
                return {
                    ...list,
                    books: list.books.filter(book => (book.ISBN !== newBook.ISBN))
                }
            }
        ...
        toast(
            <BookNotification message={ACTIONS_MSGS.ADDED_TO} book={newBook} listName={title} Icon={Icon} />
        )
    },
    deleteBook: ({ bookToDelete, listIdOrigin }) => {
        ...
        const newList = lists.map(list => {
            if(list.id === listIdOrigin) {
                return {
                    ...list,
                    books: list.book.filter(book => book.ISBN !== bookToDelete.ISBN)
                }
            }
        ...
        set({ lists: newList })
        toast(
            <BookNotification message={ACTIONS_MSGS.DELETED_FROM} book={bookToDelete} listName={title} Icon={DeleteIcon} />
        )
    }
}),
{
