...
import { DESKTOP_LARGE_DEVICE_SIZE } from '../constansts/globals'
...
export function useBooklistGridStyles({ books }) {
    ...
    // TODO --- not only calculate the rows, also do it with the columns
    const calculateRows = () => {
        if(window.innerWidth < DESKTOP_LARGE_DEVICE_SIZE) return '5rem'
        const columns=6
        const rows=Math.ceil(books.length / columns)

        // TODO --- This work but isn't the best way, because the "elementToApply"
