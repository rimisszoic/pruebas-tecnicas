...
export function BookContent({ book }) {
    ...
    const isBookAlreadyInAList = (listId) => lists.find(list => {
        return list.book.some(({ ISBN }) => ISBN === book.ISBN)
    })?.id === listId
    ...
    return (
        <>
            <img id='imagePositionTarget' src={book?.cover} className={styles.imagePositionTarget} aria-hidden={thereIsSelectedBook} />
            <div className={styles.bookContent} aria-hidden={thereIsSelectedBook}>
                {
                    thereIsSelectedBook &&
                        <>
                            ...
                            <div className={styles.listBtns}>
                                {lists.map(list => (
                                    <ActionListButton key={list.id} action={() => handleClick(list.id)} nameList={list.title.toLowerCase()} Icon={findListDetails(list.id).Icon} variant={isBookAlreadyInAList(list.id) ? 'isAlready' : ''} />
                                ))}
                            </div>
